...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('dashboard.sambutan.update', $sambutan)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                <div class="form-group">
                    <label for=""@error('isi_sambutan') id="content class="text-danger" @enderror >Isi Sambutan</label>
                    <textarea name="isi_sambutan" id="" rows="3" class="form-control my-editor @error('isi_sambutan') form-control is-invalid @enderror"
                    placeholder="Masukkan Isi Sambutan"> {{$sambutan->isi_sambutan }} </textarea>
                    @error('isi_sambutan')
