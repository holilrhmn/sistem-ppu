...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Data Tipe Kendaraan</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('dashboard.type.update', $type)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method("PUT")
              <div class="form-group">
                  ...
                  placeholder="Masukkan type" value="{{$type->name ??  old('name') }}">
                  @error('name')
              ...
              <div class="form-group">
                ...
                placeholder="Masukkan Pertanyaan" value="{{$type ?? old('jumlah') }}">
                @error('jumlah')
                    <span  class="text-danger"> {{ $message }} </span>
                @enderror
            </div>
